...
- 입력
  - 최대 200자 작성 가능
  - 비회원이 등록하기 버튼 누룰 시 로그인 페이지로 이동
- 댓글 요소
  - 작성자 / 내용 / 작성일 / 대댓글 보기 / 대댓글 등록하기
- 작성자만 수정, 삭제 버튼 보여줌
- 작성자 누룰시 해당 유저 블로그 페이지로 이동

<br/>

### 10. 유저 블로그 페이지 (/@{username})

<br/>

#### 10-1. 프로필

- 상위에 유저 프로필 이미지 & 이름 출력
...
### 11. 시리즈 상세 페이지 (/@{username}/series/{seriesId})

- 해당 시리즈에 해당되는 공개 출간 게시글만 출력
- 게시글 누르면 상세페이지로 이동
- 불러오기 버튼
  - 비동기를 통해서 추가적으로 게시글 가져옴
...
- ID / PW / 프로필 이미지 / 이름 / 이메일 출력
- ID를 제외한 내용 변경버튼 제공
- PW
  - 기존 PW / 새 PW 형식 / PW 체크 통과시 변경 가능
- EMAIL
  - 이메일 형식 통과시 변경 가능
...
- 회원 탈퇴 버튼
  - 누르면 주의 문구 및 PW 입력칸 출력
  - PW 입력시 최종 탈퇴 진행

### 13. 로그아웃

- 헤더 아이디 버튼의 로그아웃을 누르면 로그아웃 수행
